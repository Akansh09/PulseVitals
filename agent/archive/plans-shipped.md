# Shipped plans — PulseVitals

This file archives completed plan entries that previously lived in `CLAUDE.md`. They are kept verbatim for traceability (PM Spec → Release artifacts per the agent pipeline). Active plans live in `CLAUDE.md`.

Versions covered: 1.0 (Design System docs) → 2.1 (feature pass).

For runtime / behavior history, see `CHANGELOG.md`. For commit history, use `git log`.
